...
function executeRequest($historyID, $type='', $action='', $URL='', $requestData='', $timeout='', $isPluginResponse=true, $callOpt=array()){	
	...
	if(!$cURLErrors->isOk()){
		...
		$multicallArrayCheck =   array('3', '6','7', '16', '18', '28', '35', '52', '56', '92', '100', '500', '502', '504','503', '521', '522', '508', '524', '525', '520', '526', '527', '400', '405', '408', 'timeoutClear', 'main_plugin_connection_error');
		$multicallInitialCheck = array('3', '6','7', '16', '18', '28', '35', '52', '56', '92', '100', '500', '502', '504','503', '521', '522', '508', '524', '525', '526', '520', '400', '527', '405', '408', 'timeoutClear', 'main_plugin_connection_error');
		$phoenixRetryCheck =     array('3', '6','7', '16', '18', '28', '35', '52', '56', '92', '100', '500', '503', '502','504', '521', '522', '508', '524', '525', '526', '520', '527', '400', '405', '408', 'timeoutClear', 'main_plugin_connection_error');
		if(!isPluginResponse($rawResponseData)){//sometimes 500 error with proper IWP Client Response, so if it not proper response continue set error and exit
			if (in_array($updateHistoryData['error'], $phoenixRetryCheck) && triggerPhoenixBackupFailedTask($historyID, $actionResponse, $curlInfo['errorNo'], $curlInfo['info']['http_code'])) {
				return checkTriggerStatus();
			}
			if (in_array($updateHistoryData['error'], $multicallInitialCheck) && retriggerInitialMultical($historyID)) {
				return false;
			}
	...
	if($isPluginResponse){//$isPluginResponse is set to true then expecting result should be pluginResponse
		...
		if(!isPluginResponse($rawResponseData)){ //Checking the timeout	
			...
			if (($type=='stats' && $action == 'getStats') && (strpos($URL,'http://')!==false && (strpos($rawResponseHeader ,'301 Moved Permanently') || strpos($rawResponseHeader ,'302 Found')))) {
				$headers = explode("\n",$rawResponseHeader);
				$j = count($headers);
				for($i = 0; $i < $j; $i++){
					if(strpos($headers[$i],"Location:") !== false){
						$redir = strtok(trim(str_replace("Location:","",$headers[$i])), '?');
						if (strpos($redir,'.php') || strpos($redir,'http://')) {
							break;
						}
						$whereHis = array(
			      		'query' =>  "historyID=':historyID'",
	        			...
	        			$URLwithoutProtocol = str_ireplace(array('www.', 'http://', 'https://'), '', $URL);
	        			$URLwithoutProtocol = strtok(trim(str_replace("Location:","",$URLwithoutProtocol)), '?');
	        			$reURLwithoutProtocol = str_ireplace(array('www.', 'http://', 'https://'), '', $redir);
	        			if ($URL != $reURLwithoutProtocol) {
	        				break;
	        			}
						$retriedTask = array('status'=>'retry', 'URL' => $redir);
						if ($isAdminURL && strpos($redir,'https://') === false) {
							$retriedTask = array('status'=>'retry', 'URL' => $redir);
							DB::update('?:history', $retriedTask, $where);
							return true;
						}elseif (strpos($redir,'https://')!==false) {
							$where = array(
							  'query' =>  "siteID=':siteID'",
							  'params' => array(
								   ':siteID'=>$siteID
							...
							DB::update("?:sites", array('URL' => $redir), $where);
...
function retryFailedTasksChecker($historyID, $actionResponse, $errorNo = NULL, $http_code = NULL){
	...
	$needRetryhttp_code = array('500', '501', '503', '504', '524');
...
function checkTriggerStatus(){
	...
	$historyErrorCode  = array('3', '6','7', '16', '28', '18', '35','52', '56', '92' ,'96', '100', '500', '502', '504','503', '521', '522', '508', '524', '525', '526', '520', '527', '400', '405', '408', 'timeoutClear', 'processingResponseDied', 'main_plugin_connection_error', 'dropbox_error');
	$where = array(
	...
	foreach($data as $key => $value){
		if ($value['action'] == 'multiCallRestore' || $value['action'] == 'restoreNew' || $value['action'] == 'bridgeExtractMulticallRestore') {
			continue;
		}
		$where = array(
		...
		foreach($subTaskData as $subTaskKey => $subTaskValue){//trying to find three consecutive error
			if (in_array($subTaskValue['status'], $historyStatus)) {
				continue;
			}elseif (in_array($subTaskValue['status'], array('error', 'netError')) && in_array($subTaskValue['error'], $historyErrorCode)) {
				$errorCount++;
			}else{
				break;
			}
