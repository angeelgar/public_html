...
class manageClientsRestore {
	...
	public static function restoreBridgeUploadProcessor($siteIDs=array(), $params){
		...
		$events=1;
		$file = APP_ROOT."/includes/bridge/restore.php";
		...
		foreach($statusData as $d1){
			...
			foreach($d1 as $d2 => $d3){
				if($d2 == "error"){
					...
					$d3['isStaging'] = ($type == "staging") ? true : false;
		  			if( $type == 'staging'){
						self::manageClientsInstallCloneCommon($historyData['parentHistoryID'], $d3);
					}else{
						self::manageClientsInstallCloneCommon('', $d3);
