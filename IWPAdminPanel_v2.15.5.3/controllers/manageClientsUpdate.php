...
class manageClientsUpdate {
	...
	public static function updateAllResponseProcessor($historyID, $responseData){
		...
		self::triggerNextUpdateCall($historyID);
	...
	public static function isUpdateRunning($historyID){
		$whereHis = array(
		...
		usleep(2000000);
		$where = array(
			      		'query' =>  "siteID = ':siteID' AND type = 'PTC' AND status ='running'",
			      		'params' => array(
			               ':siteID'=>$siteID
           				)
        			);
	...
	public static function getUpdatePendingTasks($historyID){
		...
		$siteID = DB::getField("?:history", "siteID", $whereHis);
		$where = array(
			      		'query' =>  "siteID = ':siteID' AND type = 'PTC' AND status IN ('pending', 'scheduled')",
			      		'params' => array(
			               ':siteID'=>$siteID
           				)
        			);
